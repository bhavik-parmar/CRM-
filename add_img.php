...
<html lang="en">
  ...
  <div class="col-md-12 grid-margin stretch-card">
    <div class="card">
      <div class="card-body">
        <h4 class="card-title mb-4">Add Image</h4>
        <form action="add_img.php" method="post" enctype="multipart/form-data">
          <div class="form-group mb-3">
            <label for="leadName">Img_Name</label>
            <input type="text" class="form-control" id="leadName" name="name" placeholder="Enter Image Name" required>
          </div>
          <div class="form-group mb-3">
            <label for="leadCompany">Uploade Image</label>
            <input type="file" class="form-control" id="leadCompany" name="image" placeholder="Add Cource Broucher">
          </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $image_folder = 'gallary/' . basename($image_name);

    // Check and move file
    if (move_uploaded_file($image_tmp, $image_folder)) {
        // Insert into database
        $sql = "INSERT INTO photogallary (img_name, img)
                VALUES ('$name',  '$image_name')";

        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('Image added successfully.'); window.location.href='photogallary.php';</script>";
        } else {
